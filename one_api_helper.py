...
class OneAPIHelper:
    def __init__(self):
        ...
        if not self.api_token:
            raise ValueError("OneAPIHelper environment variable not set")
        
        self.base_url = os.getenv('OPENAI_BASE_URL') + "/chat/completions"
        self.headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_token}"
        }

    async def get_completion(self, messages, model="host-c37-sonnet-20250219", max_new_tokens=500, temperature=0.5, top_p=1, stream=False):
        """
        Get completion from OneAPI API
        ...
        try:
            ...
            logging.debug(f"Sending request to OneAPI API with data: {data}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        logging.error(f"OneAPI API error: {response.status}")
